...
def _extract_in_subprocess(pdf_path: str, max_tables: int) -> Optional[List[TableInfo]]:
    """Run extraction in a clean interpreter. Returns None if that isn't possible."""
    try:
        proc = subprocess.run(
    ...
    return [TableInfo(**row) for row in payload]
...
def extract_tables(pdf_path: str, max_tables: int = 40) -> List[TableInfo]:
    ...
    failed" are both the caller's to handle, and the failure is logged.
    """
    if _process_is_polluted():
        tables = _extract_in_subprocess(pdf_path, max_tables)
        if tables is not None:
            return tables
        logger.warning("table_extraction_degraded",
                       reason="subprocess unavailable; cell geometry may be corrupted")
...
def _extract_tables_impl(fitz, pdf_path: str, max_tables: int) -> List[TableInfo]:
...
def _main(argv: Sequence[str]) -> int:
    ...
    import fitz  # type: ignore
    tables = _extract_tables_impl(fitz, pdf_path, max_tables)
    json.dump([asdict(t) for t in tables], sys.stdout)
    return 0
